...
function getAccessToken() {
    return 'Bearer ' + fetch(config.getConfig().contextBroker.access_token_host, {
            method: 'POST',
            body: 'grant_type=vp_token&' + config.getConfig().contextBroker.VP_token + '&scope=default',
            headers: {
                'Content-Type': 'application/x-www-form-utlencoded',
                'Accept': '*/*'
            }
        }).then((responseText) => responseText.text).then((response) => {return response})
}
...
function createRequestObject(url, typeInformation, token) {
    ...
    if (config.getConfig().contextBroker.authentication) {
        cbHost = config.getConfig().contextBroker.PEP_host;
        headers['Authorization'] = 'Bearer ' + config.getConfig().contextBroker.VP_token;
        console.log();
